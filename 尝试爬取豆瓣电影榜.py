...
_content={}#保存排行榜信息index:[image,title,actor,time,score]
def download_parse(url):
    global _content
    #定义正则表达式，匹配网页中的影片的七种数据信息
    ...
    try:
        ...
        #抽取把正则表达式匹配得结果信息，并放置到全局变量_content中
        for item in items:
         board_index=item[0]
         image_url=item[1]
         name=item[2]
...
def main():
    global _content
    url_pattern='http://maoyan.com/borad/4?offset={0}'
    for i in range(0,100,100):
        url=url_pattern.format(i)
        download_parse(url)
        time.sleep(1)#延时0.5秒，避免被服务器拒绝访问
    save_json(r'd:\maoyan_top100.json')
    save_csv(r'd:\maoyan_top100.csv')
